...
export default function FavoritosPage() {
  ...
  const toggleFavorito = async (id: string, favoritoActual: boolean) => {
    try {
      ...
      if (error) throw error;
      cargarFavoritos(); // Recargar para actualizar la UI
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="ml-64 p-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Accede rápidamente a tus cálculos más importantes y útiles</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Promedio tarifa/hora</p>
                <p className="text-2xl font-bold text-gray-900">
                  {calculos.length > 0 
                    ? formatearCLP(calculos.filter(c => c.result && typeof c.result.tarifaHora === 'number').reduce((sum, c) => sum + c.result.tarifaHora, 0) / calculos.filter(c => c.result && typeof c.result.tarifaHora === 'number').length)
                    : '$0'}
                </p>
              </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Último favorito</p>
                <p className="text-lg font-semibold text-gray-900">
                  {calculos.length > 0 
                    ? new Date(calculos[0].created_at).toLocaleDateString('es-CL', { day: 'numeric', month: 'short' })
                    : 'N/A'}
                </p>
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100">
            <h2 className="text-lg font-semibold text-gray-900">Cálculos favoritos</h2>
          </div>
          {calculos.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="text-gray-600 mb-6">Marca como favorito los cálculos que más uses para acceder a ellos rápidamente</p>
              <div className="flex justify-center space-x-4">
                <Link href="/calculadora" className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors">
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                  </svg>
                  Crear cálculo
                </Link>
                <Link href="/dashboard/historial" className="inline-flex items-center px-4 py-2 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 transition-colors">Ver historial</Link>
              </div>
            </div>
          ) : (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        <h3 className="text-lg font-semibold text-gray-900">{calculo.titulo || 'Cálculo sin título'}</h3>
                    ...
                    <div className="flex items-center space-x-2 ml-6">
                      ...
                      <Link href={`/dashboard/calculo/${calculo.id}`} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium">Ver detalles</Link>
                    </div>
                  </div>
                </div>
              ))}
